...
def main():
    ...
    if cfg.data.get('support_init', None) is not None:
        model_init_dataset = build_dataset(cfg.data.support_init)
        # disable dist to make all rank get same data
        model_init_dataloader = build_dataloader(
            model_init_dataset,
            samples_per_gpu=1,
    ...
    if not distributed:
        ...
        if (cfg.data.get('model_init', None) is not None) or (cfg.data.get('support_init', None) is not None):
